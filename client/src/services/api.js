...
// NOTE: intentionally no try/catch here.
// Errors propagate to the caller (TopicForm) where they are handled and
// shown to the user. Swallowing here was the root cause of the
// "Cannot read properties of undefined (reading 'data')" crash.
export const generateNotes = async (payload) => {
  const result = await axios.post(
    ...
    {
      withCredentials: true,
      // 2-minute timeout — AI generation can be slow in production
      timeout: 120000,
    }
  );
  return result.data; // { data, noteId, creditsLeft }
};
    ...
    try {
      ...
      requestAnimationFrame(() => {
        try {
          ...
          const W = Math.max(
            rect.width || svg.viewBox?.baseVal?.width || 0,
            300
          );
          const H = Math.max(
            rect.height || svg.viewBox?.baseVal?.height || 0,
            200
          );
