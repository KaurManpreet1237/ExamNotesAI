  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        amountPaid: amount, // store INR amount for totalSpent tracking
      },
    });
...
export const stripeWebhook = async (req, res) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const amountPaid = Number(session.metadata.amountPaid) || 0;

    if (!userId || !creditsToAdd) {
      return res.status(400).json({ message: "Invalid metadata" });
    }

    // Add credits AND increment totalSpent for admin analytics
    await UserModel.findByIdAndUpdate(userId, {
      $inc: {
        credits: creditsToAdd,
        totalSpent: amountPaid, // ← tracks revenue per user for admin panel
      },
